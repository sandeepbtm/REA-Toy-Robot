...
def main():
    ...
    execute_directory = stdin.read()
    print("=============",execute_directory)
    if execute_directory == "input_data":
        correct_files = fm.load("input_data", "r")
        for correct_file in correct_files:
            print("\n")
            print("FIle executed from Correct Directory - ", correct_file)
            for values in fm.read(correct_file):
                for line in values.split("\n"):
                    if len(line):
                        simulation.run(line)

    elif execute_directory == "incorrect_data":
